...
def make_itinerary(path: list, final_arrival: int, network: transit_core.Network,
                    board_name: str, alight_name: str) -> dict:
...
def scan_from_origin(network: transit_core.Network, origin_stops: list,
                      targets: dict, board_name_override: str = None) -> dict:
...
def compute_day_type_schedules(network: transit_core.Network, districts: list,
                                destinations: list, stop_index: StopIndex) -> dict:
    """戻り値:
    ...
    for d in districts:
        ...
        outbound[d["id"]] = scan_from_origin(network, stops, facility_targets)
    ...
    for f in destinations:
        ...
        inbound[f["id"]] = scan_from_origin(network, stops, district_targets,
...
def main():
    ...
    for day_type, ref_date in REFERENCE_DATES.items():
        ...
        per_daytype[day_type] = compute_day_type_schedules(network, districts, destinations, stop_index)

...
最短ルートを持つとは限らない」。設計C = docs/handover.md §2):
  - 自宅(地区)側の候補停留所選び(build_origins, expand_by_route=True)を、距離だけ
    ...
    施設側(帰りの出発)は最寄り+同じ場所のまま(inbound肥大を防ぐ)。
...
ことへの結論):
  ...
  - JSONは整形せずコンパクトに書き出す(separators=(",",":")。人が読まない生成物なので
    可読性より約2割の減量を優先)。
  - 帰り(inbound)の便は間引かない(施設名で1停に統一済み。帰りの時刻表を薄くしない)。
...
def boardable_routes(network: transit_core.Network, stop_id: str) -> set:
    """その停留所から乗れる系統名(route_name)の集合。build_originsで
    「最寄り停には無い系統を持つ停」を見分けるのに使う(2026-07-07 開発者指摘への対応)"""
    routes = set()
    for pattern_idx, pos in network.stop_routes.get(stop_id, []):
        pattern = network.patterns[pattern_idx]
        if pos == len(pattern.stop_ids) - 1:
            continue  # 終点では乗れない
        routes.update(trip.route_name for trip in pattern.trips)
    return routes
...
def keep_useful_boards(rows: list) -> list:
    ...
    乗り場が1種類(=帰り方向は施設名で統一)のときは自然に全便が残る"""
    if len(rows) <= 1:
        return rows
...
def build_origins(network: transit_core.Network, stop_index: StopIndex, points: list,
    ...
    for pid, lat, lon, name in points:
        ...
        covered_routes = boardable_routes(network, nearest_id)
        ...
        for stop_id, walk_min in hits[1:]:
            ...
            if d <= SAME_PLACE_M:
                ...
                covered_routes = covered_routes | boardable_routes(network, stop_id)

        # (3) 別系統を持つ停だけを追加する(近いだけで系統が同じ停は増やさない)
        if expand_by_route:
            for stop_id, walk_min in hits[1:]:
                if stop_id in same_place_ids:
                    continue
                routes = boardable_routes(network, stop_id)
                new_routes = routes - covered_routes
                if new_routes:
                    chosen.append((stop_id, walk_min, name))
                    covered_routes = covered_routes | new_routes
...
def compute_day_type_schedules(network: transit_core.Network, districts: list,
    ...
    # 施設側(帰りの出発)は最寄り+同じ場所のみ。利用者が乗り場を意識するのは自宅側
    facility_origins = build_origins(network, stop_index, facility_points)
...
def build_entry(district: dict, facility: dict, per_daytype: dict) -> dict:
    ...
    # かんたん・しっかり とも同じ1停の完全な時刻表を見せる(最寄り≠最短は解決。
    # 複数停からの選択は将来F10のオンデマンド機能へ送る)。これによりデータ量が
    # 都心で爆発せず~20MBに収まる。※pick_kantan_board は全乗り場を見て選ぶ必要があるため、
    #   必ず「絞り込む前の outbound」に対して呼ぶこと(順序を入れ替えないこと)
    kantan_board = pick_kantan_board(outbound)
    if kantan_board is not None:
        entry["kantan_board"] = kantan_board
        # 行きを その1停の便だけに絞る(帰り inbound は施設名で1停に統一済みなので触らない)
        for day_type, rows in outbound.items():
            outbound[day_type] = [r for r in rows if r["board"] == kantan_board]
...
def pick_kantan_board(outbound: dict):
    ...
    for rows in outbound.values():
        for r in frontier_rows(rows):
            b = r["board"]
            totals.setdefault(b, []).append(hm_to_min(r["arr"]) - _leave_home_min(r))

...
最短ルートを持つとは限らない」。設計C = docs/handover.md §2):
  ...
  - かんたんモード: 行き先ごとに「一番いい乗り場」を1つだけ選び(pick_kantan_board →
    entry.kantan_board)、その停の完全な時刻表だけを表示する(見慣れない停を混ぜない。
    時計モードが嘘をつかないよう、選んだ停の便は間引かない)。
  - しっかりモード: outboundの全停(=有用な停)を残し、既存の「乗車バス停で絞り込み」
    (webapp/shikkari)でそのまま選ばせる。
...
def build_entry(district: dict, facility: dict, per_daytype: dict) -> dict:
    ...
    if kantan_board is not None:
        # かんたんモードは この1停の完全な時刻表だけを見せる(見慣れない停を混ぜない)。
        # しっかりモードは outbound の全停を出し、既存の乗車バス停フィルタで選ばせる

...
# 地域固有の値は region.py が管理(山形の既定値: 平日20260610=分析確定版と同じ基準日、
# 土曜20260613・日祝20260614=例外の影響がない直近の週末)
from region import REGION, is_default_region, region_expected

REFERENCE_DATES = dict(REGION["reference_dates"])
DAY_TYPE_LABELS = {"weekday": "平日", "saturday": "土曜", "sunday_holiday": "日曜・祝日"}

# date_tableの収録範囲と有効期限(山形: 山交バス・上山市営バスのcalendar.txt end_dateが
# 2026-09-30でもっとも早く切れるため、これが9フィード全体の実質的な有効期限になる)
DATE_TABLE_START = REGION["date_table_start"]
VALID_UNTIL = REGION["valid_until"]

# 基準フィード(山形: 山交バス)。calendar_dates.txtに祝日・お盆の例外を実際に
# コード化しているフィードを選び、date_typeの判定の基準にする
REFERENCE_FEED_DIR = next(
    (d for d in config.GTFS_FEED_DIRS if d.name == f"gtfs_{REGION['reference_feed']}"), None)
assert REFERENCE_FEED_DIR is not None, \
    f"基準フィード gtfs_{REGION['reference_feed']} が gtfs_feed_dirs にありません(region設定を確認)"
...
# デマンド交通・相談窓口と運行主体の連絡先は、人間が確認・編集するマスタCSVから読む
# (全国展開キットR1で外出し。山形の値は docs/demand_transport_memo.md と
#  2026-07-07のWeb調査から転記済み。★印メモの最終確認事項もCSVのmemo列にある)
DEMAND_PHONE_MASTER_CSV = config.DATA_DIR / "demand_phone_master.csv"
OPERATORS_MASTER_CSV = config.DATA_DIR / "operators_master.csv"


def load_demand_phone() -> list:
    """demand_phone_master.csv → meta.json の demand_phone 配列(行の順序を保つ)。
    district_names 列(「;」区切り)があれば対象地区のリスト、無ければ
    city_catchall 列の市町村の「その他全地区」フォールバック窓口になる。
    ファイルが無ければ空(ウィザードが後で作る。画面は電話欄なしで壊れず動く)"""
    if not DEMAND_PHONE_MASTER_CSV.exists():
        print(f"※ {DEMAND_PHONE_MASTER_CSV.name} が無いため、デマンド交通・相談窓口の"
              f"電話案内は空になります(データはそのまま使えます)")
        return []
    rows = pd.read_csv(DEMAND_PHONE_MASTER_CSV, dtype=str).fillna("")
    entries = []
    for r in rows.itertuples():
        if r.district_names:
            districts = [d.strip() for d in r.district_names.split(";") if d.strip()]
        else:
            districts = f"{r.city_catchall}のその他全地区"
        entries.append({"name": r.name, "tel": r.tel, "districts": districts})
    return entries


def load_operator_contact() -> dict:
    """operators_master.csv → {フィード名: {name, desk, tel}}。
    キーはフィード名(= trip_id の「◯◯:」接頭辞。gtfs_◯◯ ディレクトリ名から)。
    tel が空欄なら None(画面には運行主体の名前だけ出て、電話番号は出ない)。
    ファイルが無ければ空(フィード名がそのまま表示名になる)"""
    if not OPERATORS_MASTER_CSV.exists():
        print(f"※ {OPERATORS_MASTER_CSV.name} が無いため、運行主体の表示名は"
              f"フィード名のまま・電話番号なしになります")
        return {}
    rows = pd.read_csv(OPERATORS_MASTER_CSV, dtype=str).fillna("")
    return {r.feed: {"name": r.name or r.feed, "desk": r.desk or None, "tel": r.tel or None}
            for r in rows.itertuples()}


DEMAND_PHONE = load_demand_phone()
OPERATOR_CONTACT = load_operator_contact()
...
def main():
    ...
    # 検算1: 祝日・お盆のダイヤ種別(期待値は地域設定に持つ。山形: 海の日とお盆が日祝)。
    # 期待値の無い地域では、祝日の判定結果を数件表示して人間が確かめられるようにする
    date_checks = region_expected("date_checks")
    if date_checks:
        for d, want in date_checks:
            print(f"  検算1: {d} = {date_table.get(d)}(期待値 {want})")
    else:
        holidays = [(d, t) for d, t in sorted(date_table.items())
                    if jpholiday.is_holiday(date.fromisoformat(d))][:4]
        print(f"  検算1(この地域の期待値は未設定): 祝日の判定例 = {holidays}"
              f" … 祝日が sunday_holiday 相当になっているか目視確認してください")
